...
try:
    logo = Image.open("header_logo.png")  # keep this file next to app.py if you want to show it
    st.image(logo, width=300)
except Exception:
    pass  # don't fail if logo isn't present
...
def build_output_table(vf_tidy: pd.DataFrame, counts: pd.DataFrame) -> pd.DataFrame:
    ...
    for center, group in merged.groupby("Center", sort=True):
        # Class rows (Applied/Accepted blank)
        ...
        # Center Total row (Applied/Accepted filled)
