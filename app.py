...
st.markdown("Upload your **Enrollment.xlsx** file to receive a formatted version (hard-coded class names applied).")
...
# Build a normalized-key version of the hard-coded map (CRITICAL for matching)
...
if uploaded_file:
    ...
    # ----------------------------
    # HARD-CODED CLASS OVERRIDE (only functional change)
    # ----------------------------
    ...
    if center_col is None:
        ...
        for e_center in df["__CenterClean"].dropna().unique():
            key = norm_center(e_center)               # normalize center like the dict
    ...
    # ----------------------------
    # 4) Style with openpyxl (original)
    # ----------------------------
